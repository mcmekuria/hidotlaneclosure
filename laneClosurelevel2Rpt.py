...
qry = "Route is null"
sectqry = wmlnclyrsects.query(where=qry)
...
lndelete = "enDate > beginDate " # + '{}' )".format( timedelta(days=7) ) # and EditDate>= '{}'".format((datetime.datetime.today()-timedelta(days=1)).strftime("%m/%d/%y"))

# prepare the global Id's that have been updated using the comparison creation and update dates 
# get the unique level2 approvers list 
uniquel2 = sectsdf['ApproverL2'].value_counts()

# write a report of the sections that are to be emailed to Level 2 approvers 
for l2id in uniquel2.items():
    l2 = l2id[0]
    l2qry0 = "'{}' in ApproverL2 & ApprLevel1 == '{}' ".format(l2,'Yes')
    sectsdfl2id0 = sectsdf.query(l2qry0 )
    #l2qry1 = "ApprLevel1 == '{}' ".format('Yes')
    #sectsdfl2id1 = sectsdfl2id0.query(l2qry1 )
    if len(sectsdfl2id0)>0:
        sectsdfl2idm = sectsdf[sectsdf['Route'].notnull()] 
        sectsdfl2idm.set_index(['Route'],inplace=True) 
        sectsdfl2idms = sectsdfl2idm.sort_index(ascending=False)
        sectsdfl2idx = sectsdfl2idms.head(1)
        dfcols = sectsdfl2idm.columns
        unrtesel = sectsdfl2id['Route'].value_counts()
         
        #    sectsdfl2id = projsdfmp.query("routeno == \'" + rt[0] + "\'" )
        print("Rte {} ; no recs : {} ".format(unrtesel[0],unrtesel[1]))
        ix = 0
        for idx,srow in sectsdfl2idms.iterrows():
            emp = srow.emp
            bmp = srow.bmp
            rte = srow.Route
            ...
            xsbedte = srow.beginDate
            xsbedtes = datetime.datetime.timestamp(xsbedte) # + datetime.timedelta(int(utcofs)))
            xsendte = srow.enDate
            xsendtes = datetime.datetime.timestamp(xsendte) # + datetime.timedelta(int(utcofs)))
            print(' report section {} ; Route : {}; pgid : {} ;  gid : {} ; Creator : {} ; Created : {} ; edited : {} ; deleted : {} \n '.format(
                        srow.objectid,srow.Route,srow.parentglobalid , srow.globalid, srow.Creator, srow.EditDate, srow.beginDate,srow.enDate))
